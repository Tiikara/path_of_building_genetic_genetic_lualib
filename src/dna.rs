...
impl Dna {
    ...
    pub fn mutate(mut self, rng: &mut ThreadRng) {
        ...
        let mut body_slice = &self.body[start_num..start_num+mutate_cluster_size];
    ...
    fn crossover_dna(dna1: &Dna, dna2: &Dna, range: RangeInclusive<usize>) -> Dna {
        let new_dna = dna1.clone_dna();

        new_dna.body[range].clone_from_slice(&dna2.body[range.clone()]);
